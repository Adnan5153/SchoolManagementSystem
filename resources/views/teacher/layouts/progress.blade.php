...
@section('content')
    ...
    @foreach ($groupedMarks as $groupKey => $group)
        @php
            $student = $group->first()->allstudent;
            [$className, $section] = explode('-', $groupKey);
        @endphp

        <div class="card shadow mb-4 w-100">
            <div class="card-header bg-dark text-white">
                <h5 class="mb-0">
                    <i class="fa-solid fa-chart-line"></i> Student Performance: {{ $className }} - {{ $section }}
                </h5>
            </div>
                        ...
                        <tbody>
                            @foreach ($group as $mark)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <!-- ✏️ Edit Button -->
                                        <button class="btn btn-sm" data-bs-toggle="modal" data-bs-target="#editModal{{ $mark->id }}">
                                            <i class="fa-solid fa-pen-to-square" style="color: #FFD43B;"></i>
                                        </button>

                                        <!-- 🗑️ Delete Form -->
                                        <form action="{{ route('progress.destroy', $mark->id) }}" method="POST" class="d-inline"
                                            onsubmit="return confirm('Are you sure you want to delete this mark?')">
                                            @csrf
                                            @method('DELETE')
                                            <button class="btn btn-sm"><i class="fa-solid fa-trash" style="color: #ff0000;"></i></button>
                                        </form>
                                        ...
                                        <div class="modal fade" id="editModal{{ $mark->id }}" tabindex="-1" aria-labelledby="editModalLabel{{ $mark->id }}" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered">
                                                <div class="modal-content">
                                                    <form action="{{ route('progress.update', $mark->id) }}" method="POST">
                                                        @csrf
                                                        @method('PUT')
                                                        <div class="modal-header bg-primary text-white">
                                                            <h5 class="modal-title" id="editModalLabel{{ $mark->id }}">
                                                                Edit Marks for {{ $student->first_name }} {{ $student->last_name }}
                                                            </h5>
                                                        ...
                                                        <div class="modal-body">
                                                            <div class="mb-3">
                                                                ...
                                                                <input type="number" name="marks" class="form-control" value="{{ $mark->marks }}" required min="0" max="100">
                                                            </div>
                                                            <div class="mb-3">
